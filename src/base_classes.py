...
class ODEFunc(MessagePassing):
  ...
  def __init__(self, opt, data, device):
    ...
    if self.opt['channel_mixing']:
      self.alpha_train = nn.Parameter(opt['init_alpha']*torch.ones(opt['hidden_dim'],opt['hidden_dim']))
    else:
      self.alpha_train = nn.Parameter(opt['init_alpha']*torch.ones(1,opt['hidden_dim']))
    self.delta_train = nn.Parameter(opt['init_delta']*torch.ones(1,opt['hidden_dim']))
